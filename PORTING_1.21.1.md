...
| [x] | Screen | `common/.../screens/**`, `deprecated/screens/**` | `1.16.5 .../screens/**`; 1.12 GUI packages | `src/client/.../screens/**` | Yes, substantial | Theme snapshot, GuiGraphics, vanilla widgets | Client-only | Phase seven implements the SAO screen framework while retaining vanilla menu/container ownership and modded-screen safety; awaiting manual user validation. |
...
| [x] | Mixins | `common/.../mixin/ModConfigMixin.java`, Fabric mixin JSON | 1.16.5/1.12 mixins/ATs | `src/client/java/be/bluexin/mcui/mixin/client/**` | Yes | Mixin | Client-only | HUD suppression plus exact screen routing and policy-gated button, slider, edit-box, background, and container chrome are implemented. Mod subclasses remain outside router and generic-skin ownership. |
...
| [x] | Assets/theme packs | `common/src/main/resources/assets/{mcui,saoui}/**` | 1.16.5/1.12 full asset history | `src/main/resources/assets/{mcui,saoui}/**` | Yes, modern JSON definitions | ResourceManager theme loader | Client resources | `mcui:saoui_reborn` supplies both HUD and `screens.json`; player resource-pack priority is respected and the original SAO assets remain at their namespaced paths. |
...
Development chooses `mcui:development_test`; phase five changes production to `mcui:saoui_reborn`. If the preferred production theme is absent or invalid, the manager renders the empty fallback. No debug text is selected for ordinary production users.
...
Status: **Implemented and manually validated by the user.** Health, food, experience, hotbar, air, mount health, jump, and dynamic HUD updates were confirmed in game. No client launch, world entry, resource-pack test, screenshot test, key simulation, or automated test execution was performed by Codex in this phase.
...
Modern `hud.json` accepts an inline `fragments` map. `fragment_reference` resolves during theme compilation, wraps a newly compiled subtree at each reference transform, and leaves no per-frame registry lookup. Missing fragments are compile errors. A stack of fragment names reports circular chains and stops recursion. Child order, nested groups, transforms, and enabled flags remain intact.

The compatibility boundary is split by responsibility under `themes/legacy/`:

- `LegacyJsonThemeLoader` parses `JsonElement` with kotlinx.serialization; Gson was not reintroduced.
- `LegacyThemeAdapter` coordinates legacy document/fragment conversion.
- `LegacyHudAdapter` converts `parts` and keeps unknown names as explicit warnings.
- `LegacyPartNameMapper` normalizes historical aliases into the modern `HudPartType` enum.
- `LegacyElementAdapter` converts supported key-discriminator nodes into modern definitions.

Metadata-less `themes/<id>/hud.json` resources now run through this adapter during resource reload, receive synthetic compatibility metadata, then follow the normal `ThemeCompiler -> ResolvedHud` path. Metadata-backed `mcui:alpha` JSON also falls back to the adapter when it is not already modern schema. Conversion never occurs during a HUD frame.

Supported legacy foundations are `Hud`/parts, `ElementGroup`/`Group`, `GLRectangle`, `GLString`, texture/image aliases, `Fragment`, `FragmentReference`, standalone static-slot `GLHotbarItem`, and the standard nine-slot `RepetitionGroup + GLHotbarItem` hotbar pattern. Static numeric, boolean, position, scale, UV, alpha/color, text, and texture values are migrated. Legacy RGBA is explicitly converted to modern ARGB. Recognized health/food/air/XP/jump width expressions become typed progress bindings; recognized player name, level, health, food, air, and armor text expressions become typed text/value sources.

Unknown elements, unknown part names, unsupported repetition patterns, raw draw calls, fragment variables, and dynamic expressions produce resource/theme/field warnings. They are not evaluated. A legacy part with no supported static children is omitted so Vanilla remains available instead of being silently suppressed.

The official bundled XML theme was manually represented as modern JSON because runtime XML would require restoring xmlutil and the legacy object/expression graph. External legacy XML support remains deferred and reports an explicit loader issue. MiniScript/JEL values are limited to static literals and the small recognized typed-binding patterns above; every other expression is deferred with a warning. Koin, KSP, Lua, JEL, MiniScript, CSS, and old GL drawing remain absent.
...
The initial static screen layer consists of:
...
- `origin/1.16.5` provides the most complete five-category in-game menu, icon states, popup composition, and death artwork. Its `MainMenu` is empty and `InventoryGui` is commented out, so neither is a safe implementation source.
- `origin/2.0-1.12-features` adds a light `GuiMainMenu` extension, category entrance animation, and animated popup panels, but its inventory remains an unfinished shell. The modern implementation restores those visual intentions without the old element input system or GL calls.
- `origin/2.0-1.19.4-port` moves toward a `Screen`-based abstraction but leaves its menu, inventory, and death implementations deprecated or commented. It confirms the direction toward a client-owned screen boundary, not reusable finished behavior.

### Screen theme and immutable snapshot

- **ScreenThemeDefinition — Implemented, awaiting manual user validation.** Literal-only Kotlin Serialization DTOs define colors for panel/background/button hover/focus/pressed/disabled/selected/accent/text/muted text/slot/dialog/title/icons, texture references, opacity, padding, margin, borders, and short animation timings. Screen layout coordinates remain in Kotlin and no script, CSS, Lua, MiniScript, or JEL runtime was introduced.
- **ResolvedScreenTheme — Implemented, awaiting manual user validation.** `ThemeCompiler` resolves and validates the screen document during client resource reload. `ResolvedTheme` carries the immutable screen-style snapshot beside its HUD snapshot, and `SaoUiStyle` reads only the active snapshot while rendering.
- **Resource path — Implemented, awaiting manual user validation.** The bundled `mcui:saoui_reborn` definition is `assets/mcui/themes/saoui_reborn/screens.json`, beside `theme.mcui.json` and `hud.json`. Discovery uses `ResourceManager.getResource`, so selected resource-pack priority is preserved.
- **Fallback — Implemented, awaiting manual user validation.** A missing `screens.json` uses `ResolvedScreenTheme.FALLBACK`. Malformed JSON, unsupported format, invalid resource locations, missing textures, or invalid opacity produce diagnostics and retain the built-in SAO screen style/texture fallback instead of invalidating the HUD theme or blocking core screens.
...
### Vanilla skin integration

- **SaoUiStyle — Implemented, awaiting manual user validation.** It remains the immediate-mode render helper for menu/world backgrounds, panels, buttons, sliders, edit boxes, container frames, slots, icons, text fitting, and alpha composition. Hard-coded values moved to `ResolvedScreenTheme`; only the built-in fallback definition retains SAO defaults.
- **Buttons — Implemented, awaiting manual user validation.** `AbstractButtonSkinMixin` changes only `renderWidget` and exposes normal, hover, keyboard-focus, mouse-pressed, and disabled visuals from the screen theme. Click handling, Enter/Space, Tab order, sound, tooltip, and narration stay in `AbstractButton`/`AbstractWidget`. `SaoIconButton` uses the same renderer and adds selected-category state.
- **Sliders — Implemented, awaiting manual user validation.** `AbstractSliderButtonSkinMixin` changes only background/track/thumb rendering. Vanilla value calculation, mouse drag, keyboard adjustment, sound, message update, applyValue, and narration remain untouched.
- **EditBox — Implemented, awaiting manual user validation.** `EditBoxChromeMixin` redirects only the vanilla bordered-background sprite to a theme fill and adds hover/focus/disabled chrome after text rendering. Typing, selection, cursor, clipboard, IME delivery, max length, suggestion, formatter, and focus logic remain vanilla-owned.
- **Container chrome — Implemented, awaiting manual user validation.** `AbstractContainerScreenChromeMixin` adds an outer panel behind vanilla container textures, themed slot chrome before item rendering, and themed base labels. It never creates replacement container screens. Anvil, Beacon, Enchantment, and Smithing use frame-only mode so their specialized widgets, text, buttons, and progress rendering are not covered by generic slot decoration.
- **Background policy — Implemented, awaiting manual user validation.** SAO title/menu surfaces use the menu background, in-world SAO overlays use a dark translucent world layer, and containers retain the vanilla in-world background before their frame. The `Screen.renderMenuBackground` hook runs only for MCUI-owned or Mojang-owned screens when no world is active; arbitrary mod screens are not recolored.
- **Layering and accessibility — Implemented, awaiting manual user validation.** Container panel/frame rendering occurs before subclass backgrounds and slot decoration occurs immediately before vanilla item rendering. `InventoryScreen.renderTooltip` and recipe-book tooltip passes remain last. The framework does not replace focus paths, narratable entries, key handlers, or vanilla widgets.

### Parent routing and compatibility boundaries

- Title options, language, accessibility, world selection, and multiplayer receive the SAO title as parent. Pause options, language, accessibility, stats, advancements, social interactions, notice, and confirmation screens receive the SAO pause/death parent as appropriate, so ESC returns to the originating SAO surface.
- Widget, menu-background, and container Mixins consult `SaoScreenPolicy`. They apply to `SaoScreenSurface` or a runtime class in `net.minecraft.client.gui.screens.*`; a mod-owned subclass has a different class package and remains visually and behaviorally untouched unless it deliberately uses an MCUI screen.
- No new Mixin class was added. `MinecraftScreenRouterMixin` and `DeathScreenAccessor` were audited and retained unchanged. `AbstractButtonSkinMixin`, `AbstractSliderButtonSkinMixin`, `EditBoxChromeMixin`, `AbstractContainerScreenChromeMixin`, and `ScreenBackgroundMixin` were policy/theme-aware refactors.

### Phase-seven compile record

- `\.\gradlew.bat compileKotlin compileClientKotlin --no-daemon --no-parallel` completed successfully after one correction for Mojang's optional Realms UI class on the configured Kotlin client classpath. The retained behavior sends a disconnected Realm session to the SAO title screen; direct Realms-screen construction remains deferred with the other private/optional title integrations.
- Runtime appearance, resource reload, input, recipe book, special containers, Hardcore spectating, multiplayer restrictions, narration, and mod-compatibility remain **awaiting manual user validation**.
