...
The current authority is `origin/2.0-1.12.2-ElementRework` at `892a40a2a93d8685120d79c47b79515bab9567d8`, per the user's 1.12.2 restoration request. See [the current restoration tracker](RESTORATION_1.12.2.md).

IMPORTANT: The tables below record the PREVIOUS 1.16.5-based implementation. They are retained as historical notes, not current acceptance evidence. In particular their claims about disabled inventory/friends/navigation, popup movement, resource paths, and exact parity are superseded. No full 1.12.2 visual or functional parity is claimed.
...
|---|---|---|---|---|---|---|---|---|---|---|
| Ingame menu root | `screens/menus/IngameMenu.kt`; `screens/CoreGui.kt` | none | `(width / 2 - 10, (height - elementCount * 20) / 2)` | root transform, no panel | all descendants relative to root | none | 1.12 top-level position spread: `20f`, `easeInOutQuint`; 1.16 removed it | `orb_dropdown` on init | Relative legacy element tree; no fixed backdrop/action panel; restored 1.12 entry motion with GUI-pixel snapping | `[ADAPTED]` combines the later tree with the earlier evidenced entrance animation |
...
| Menu tree | `themes/sao/menu.xml`; runtime `IngameMenu.getDefaultElements()` | original menu icons | rooted under five categories | original element metrics | recursive | original icons/slot | original category sequencing | original menu sounds | PROFILE→SKILLS/profile; SOCIAL→GUILD/PARTY/FRIEND; MESSAGE; NAVIGATION; SETTINGS→OPTIONS/MENU/LOGOUT | `[ADAPTED]` — runtime/XML conflicts and unavailable non-party integrations are recorded below |
| Disabled/WIP state | `menu.xml`; `IngameMenu.kt` | original icons | original tree positions | original element metrics | n/a | original | no invented transition | none | SKILLS, GUILD, FRIEND, MESSAGE and NAVIGATION remain disabled where no modern backend exists; PARTY is enabled only after the server SAOMCLib handshake | `[ADAPTED]` compatibility; original grey state `[EXACT]` |
| Menu hover/focus | `IconElement.kt`; `IconLabelElement.kt`; `style.css` | `sao/gui.png`, icon assets | element bounds | unchanged | n/a | same | no invented glow | none | normal `#FFFFFF/#888888`, hover `#C99B13/#FFFFFF`, disabled `#7C7C7C/#FFFFFF`; non-focus backgrounds/icons keep the legacy half opacity, while normal text remains grey instead of reproducing the legacy white-on-white readability defect | `[ADAPTED]` readability; geometry/palette `[EXACT]` |
| Mouse-over glint | `IconElement.MOUSE_OVER_EFFECT`; legacy timing helper | enchanted-item glint | clipped to hovered element | element bounds | n/a | Minecraft glint texture | two texture-matrix passes, scale 8, rotations -50/+10, periods 3000/4873ms | none | Two-pass clipped moving glint with render color, blend and texture-matrix state restoration | `[ADAPTED]` modern render-state safety; timing/transform `[EXACT]` |
...
| Profile content | `ProfileElement.kt`; `IconElement.drawChildren()` | `menu/parts/profilebg.png`; `sao/gui.png`; armor icons | source declares `(-190,-153)`; the exact 40px correction derived from the PNG's pointer center aligns it with PROFILE, producing final offset `(-165,-93)` after the legacy child transform | `165×256`; entity size `40` | non-listed, so it consumes no row/cap but receives the accumulated child transform | direct 330×512 crop from the 512×512 PNG; shadow `(200,85,56,30)` | original `move()` cancels animation | none | Panel/shadow/name/stats stay in Background pass and entity stays in Draw pass; half-alpha panel with opaque `#555555` non-focus text, mouse-height shadow, mount-first model choice, and view deltas (`screenWidth/3.5`, `20`) are restored; modern stats substitute unavailable fields | `[PARTIAL]` — old PlayerStats/SAOMCLib capability fields are unavailable; pointer placement `[ADAPTED]` |
...
| NotificationAlert | `screens/util/NotificationAlert.kt`; `screens/SaoPartyEvents.kt` | original menu icon; vanilla toast background | toast-stack position | `160×32` | icon `(6,8)`; text X `25` | legacy toast slice `(0,96,160,32)` | shown for `5000ms` | `message` on first draw | Dedicated modern Toast with original one/two-line Y positions (`12` or `7/18`) and colors; SAOMCLib invite/join/leave/kick/disband/leader-change producers restored | visual/geometry `[ADAPTED]` to named modern toast sprite; party event behavior `[ADAPTED]` |
...
| Inventory | `screens/menus/InventoryGui.kt`; `ContainerElements.kt`; `CraftingElement.kt` | `profilebg.png` is evidence for ProfileElement, not a completed inventory screen; `slot.png` is an 84×18 menu-label atlas, not an inventory-slot texture | active original layout absent/commented | unknown | unknown | unknown | unknown | unknown | A functional vanilla-owned `InventoryScreen` is always opened. With Default Inventory disabled it still keeps the complete vanilla texture, slot coordinates, recipe book and networking. Menu atlases are never stretched over its slots. The broken historical cancellation is not retained. | `[ADAPTED]` functional compatibility; custom historical layout `[UNKNOWN]` |
...
| PARTY | `themes/sao/hud.xml`; original SAOMCLib `PartyCapability` | `textures/sao/gui.png` | root `(2,35)`; rows `y=15i` | five-slice row `162×15`; HP fill `98×3 @ (58,5)` | name `(13,8)`; Y step 15 | row slices `(0,60,10,15)`, `(10,60,4,15)`, `(14,60,43,15)`, `(57,60,100,15)`, `(157,60,5,15)`; HP `(0,188,215,9)` | none | none | SAOMCLib immutable party snapshot, original five-slice rows, health colors, eight-character names, creative/offline invalid-state tint, and hide-offline option | data/network boundary `[ADAPTED]`; geometry/UV/colors `[EXACT]` |
...
| SOCIAL → GUILD / PARTY / FRIEND | reproduced; PARTY restores SAOMCLib invite/member/kick/leave/accept/decline actions after server handshake, while unavailable GUILD/FRIEND backends stay disabled |
